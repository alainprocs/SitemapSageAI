...
# Define if we're in deployment mode
IS_DEPLOYED = os.environ.get("REPLIT_DEPLOYMENT") == "1"

# Use a flag to determine if we should use OpenAI or fallback to mocked responses
# Always use real OpenAI in development, but allow fallback in production
...
# Create the OpenAI client only if we have the module and a valid key
if HAS_OPENAI and OPENAI_API_KEY and not USE_MOCK_RESPONSES:
    try:
        openai = OpenAI(
...
def identify_topical_clusters(urls, sitemap_stats):
    ...
    If OpenAI is not available, use the mock clusters.
    ...
    """
    # If we're using mock responses, return those instead of calling OpenAI
    ...
    try:
        ...
        logger.info("Using mock clusters due to API connection error")
        # Use our pre-defined mock clusters
        return get_mock_clusters(sitemap_stats['main_domain'], sitemap_stats['total_urls'])
    
    except RateLimitError as e:
        logger.error(f"OpenAI API rate limit exceeded: {str(e)}")
        # Use mock clusters for rate limiting too
        logger.info("Using mock clusters due to API rate limit")
        return get_mock_clusters(sitemap_stats['main_domain'], sitemap_stats['total_urls'])
    
    except Exception as e:
        logger.error(f"Error in OpenAI analysis: {str(e)}")
        # Fall back to mock data for any other error
        logger.info("Using mock clusters due to general error")
        return get_mock_clusters(sitemap_stats['main_domain'], sitemap_stats['total_urls'])
